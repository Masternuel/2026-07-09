...
import { allowedExternalImage, IMAGE_CSP, MAX_IMAGE_BYTES } from "../../shared/imagePolicy.mjs";
...
test("CSP consistente em desenvolvimento, build e Vercel bloqueia carregamento externo direto", async () => {
  ...
  assert.equal(vercel.headers[0].headers.find((header) => header.key === "Content-Security-Policy").value, IMAGE_CSP);
  assert.equal((vite.match(/'Content-Security-Policy': IMAGE_CSP/g) ?? []).length, 2);
