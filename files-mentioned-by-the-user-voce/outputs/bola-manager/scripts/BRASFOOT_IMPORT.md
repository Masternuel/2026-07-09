...
Quando `FIREBASE_STORAGE_BUCKET` esta configurado, o commit envia os escudos encontrados ao Storage e grava `crestImageUrl`/`crestImagePath` no clube. Falhas individuais ficam no relatorio e nao cancelam os dados. Sem bucket, o importador registra um warning claro. Use `--skip-assets` para desativar os uploads de proposito; camisas nao sao enviadas.
